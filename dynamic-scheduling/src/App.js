...
const UserAvailability = () => {
  ...
  useEffect(() => {
    const fetchAvailability = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:8080/api/availability/${email}`, {
          headers: {
  ...
  const handleAddOrUpdateSlot = async (event) => {
    ...
    try {
      if (editIndex !== null) {
        ...
        const response = await axios.put(`http://localhost:8080/api/availability/update/${availability[editIndex]._id}`, {
          user: email,
          start: startTime,
          end: endTime,
          duration,
        ...
        // Add new slot
        const response = await axios.post('http://localhost:8080/api/availability/add', {
          user: email,
          start: startTime,
          end: endTime,
          duration,
  ...
  const handleDeleteSlot = async (index) => {
    ...
    try {
      await axios.delete(`http://localhost:8080/api/availability/delete/${slotId}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
        },
      });
